...
class com_restfulInstallerScript
{
	...
	/**
	 * Method to unzip WS folder on project root
	 */
	private function unzipWSFolder()
	{
		$zip = new ZipArchive;
		if ($zip->open(JPATH_ROOT . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_restful' . DIRECTORY_SEPARATOR . 'ws.zip') === true) {
			$zip->extractTo(JPATH_ROOT);
			$zip->close();
		}
		JFile::delete(JPATH_ROOT . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_restful' . DIRECTORY_SEPARATOR . 'ws.zip');

		if (JPATH_ROOT . DIRECTORY_SEPARATOR . '__MACOSX') {
			JFolder::delete(JPATH_ROOT . DIRECTORY_SEPARATOR . '__MACOSX');
		}
	}
	...
	private function deleteWSFolder()
	{
		JFolder::delete(JPATH_ROOT . DIRECTORY_SEPARATOR . 'ws');
	}
}
