...
require_once(JPATH_ROOT . '/ws/externalsender/ExternalSender.php');
...
class RestfulModelExternalsender extends JModelAdmin
{
	...
	/**
	 * Generating Three MySQL Triggers (AFTER INSERT, UPDATE AND DELETE)
	 * @param object|array|mixed $item = form item submitted
	 */
	public function generateTriggers($item)
	{
		$trigger = new Trigger();
		$methods = ['INSERT', 'UPDATE', 'DELETE'];
		foreach ($methods as $method) {
			$trigger->createTrigger($item->table, $trigger->generateName($method, $item->table), $method);
		}
	}
}
