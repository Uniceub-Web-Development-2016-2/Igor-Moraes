...
class RestfulModelResources extends JModelList
{
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem) {
			...
			$options = explode(',', $oneItem->columns);
			...
			$oneItem->columns = !empty($options_text) ? implode(', ', $options_text) : $oneItem->columns;
		}
		return $items;
	}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering columns
		$this->setState('filter.columns',
			$app->getUserStateFromRequest($this->context . '.filter.columns', 'filter_columns', '', 'string'));
	...
	protected function getListQuery()
	{
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$query->where('( a.table LIKE ' . $search . '  OR  a.privileges LIKE ' . $search . '  OR  a.columns LIKE ' . $search . ' )');
			}
		}
		...
		//Filtering columns
		$filter_columns = $this->state->get("filter.columns");
		if ($filter_columns) {
			$query->where("a.`columns` = '" . $db->escape($filter_columns) . "'");
		}
